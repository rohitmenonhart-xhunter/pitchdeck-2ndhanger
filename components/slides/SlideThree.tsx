...
const FLOW = [
    'Order via App/WhatsApp',
    'Van picks up from doorstep',
    'Hub presses at 150/hr',
            ...
            <div className="max-w-5xl mx-auto w-full px-4 sm:px-6">
                <div className="text-center mb-3 sm:mb-5 md:mb-6">
                    ...
                    <h2 className="font-display text-xl sm:text-3xl md:text-4xl text-[#1A1A1A] animate-slide-up">
                        ₹1 more. <span className="text-[#FF3366]">10x better.</span>
                    </h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 max-w-4xl mx-auto mb-4 sm:mb-5">
                    <div className="neo-card-flat p-3 sm:p-4 md:p-5 bg-[#F5F0E8] animate-slide-right animate-delay-2">
                        <h3 className="font-display text-sm sm:text-base text-[#8A8A8A] mb-2 sm:mb-3">Dhobi</h3>
                        <div className="space-y-1 sm:space-y-1.5">
                            {['₹10/piece', 'Coal iron', 'Burns risk', '2-day wait', 'No tracking'].map((item, i) => (
                                <div key={i} className="flex items-center gap-2 py-0.5">
                                    <span className="font-mono text-[10px] sm:text-xs text-[#FF3366] font-bold">✕</span>
                    ...
                    <div className="neo-card-solid-pink p-3 sm:p-4 md:p-5 shadow-[5px_5px_0px_#1A1A1A] animate-slide-left animate-delay-3">
                        ...
                        <div className="space-y-1 sm:space-y-1.5">
                            {['₹11/piece', 'Steam press', '0.1% damage rate', 'Same-day doorstep', 'App tracked'].map((item, i) => (
                                <div key={i} className="flex items-center gap-2 py-0.5">
                ...
                {/* Flow */}
                <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 mb-4 animate-fade-in animate-delay-5">
                    {FLOW.map((step, i) => (
                        <div key={i} className="flex items-center gap-1.5 sm:gap-2">
                            ...
                            {i < FLOW.length - 1 && <span className="text-[#FF3366] font-bold text-xs hidden sm:inline">→</span>}
                        </div>
                    ))}
                </div>

                {/* Damage honesty */}
                <div className="neo-card-solid-yellow p-2.5 sm:p-3 max-w-2xl mx-auto text-center animate-scale-in animate-delay-7 shadow-[3px_3px_0px_#1A1A1A]">
                    <p className="text-[9px] sm:text-xs font-bold text-[#1A1A1A]">
                        Damage honesty: 0.1% rate vs dhobi 2-5%. <span className="text-[#FF3366]">₹5,000/month compensation reserve in OPEX.</span> Any damage, we pay.
                    </p>
                </div>
            </div>
        </div>
